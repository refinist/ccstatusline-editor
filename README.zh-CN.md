...
- **双向编辑** —— 在编辑器里生成配置后，用配套的 `ccsa` CLI 写入本地；之后也能用 `ccsa export` 把当前配置读回编辑器继续改。
- **多语言** —— 英文、简体中文、繁體中文。
...
```bash
pnpm worker:dev      # Worker + 本地 KV 模拟 (http://localhost:8787)
```
...
- [refinist/ccstatusline-apply](https://github.com/refinist/ccstatusline-apply) —— `ccsa`，配套 CLI，负责把编辑器里生成的配置写入本机的 `~/.config/ccstatusline/settings.json`。
