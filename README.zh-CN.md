...
- **主题轮换** —— 攒一组主题，让状态栏按计划在它们之间自动切换（每小时 / 每天 / 每周 / 自定义间隔），也可以给每个星期几固定一套样式。可以顺序循环或随机切换，再把主题池导出成轮换包，用 `@refinist/ccsa` CLI 的 `rotate on` 应用。仓库里的 [`ccsa-rotation-rainbow-week.json`](ccsa-rotation-rainbow-week.json) 就是一个开箱即用的示例——「彩虹一周」，7 套主题每天换一种渐变配色，还附带当天的心情文案。
- **多语言** —— 英文、简体中文、繁體中文。
